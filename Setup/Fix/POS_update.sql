...
/****** Object:  StoredProcedure [dbo].[POS_USP_CU_USER]    Script Date: 26-09-2023 19:01:30 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROC [dbo].[POS_USP_CU_USER]
(
	@User POS_TBLUSERTYPE READONLY
)
AS
BEGIN
	UPDATE U
	...
	WHERE ISNULL(U.UPDATEDDATE, '1900-01-01') < UU.UPDATEDDATE


	INSERT INTO POS_TBLUSER(USERID, ROLEID, REPORTINGLEADID, CATEGORYID, BRANCHID, USERNAME, PASSWORDSTRING, FULLNAME, CNUMBER, EMAIL, ISOTP, GENDER, DOB, CREATEDDATE, UPDATEDDATE, DELETEDDATE)
	...
	WHERE NOT EXISTS (SELECT 1 FROM POS_TBLUSER IU WHERE IU.USERID = UR.USERID)
END
GO


UPDATE TBLCONFIG SET CONFIGVALUE = '1.4.1' WHERE CONFIGNAME = 'DBVersion'
GO