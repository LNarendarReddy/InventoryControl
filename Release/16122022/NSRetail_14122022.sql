...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM ITEMPRICE WHERE ITEMPRICEID = @PARENTITEMPRICEID AND SALEPRICE = @SALEPRICE)
	BEGIN
		SELECT 'Saleprice already exists! Enter different value to create new one.'
		RETURN;
	END

	IF NOT EXISTS(SELECT 1 FROM ITEMPRICE WHERE ITEMPRICEID = @ITEMPRICEID AND SALEPRICE = @SALEPRICE)
	BEGIN
...
BEGIN          
	...
	FROM POS_BREFUNDDETAIL BRD          
		...
		LEFT JOIN (SELECT ICPINNER.ITEMPRICEID, MAX(ICPINNER.ITEMCOSTPRICEID) AS LASTCOSTPRICEID
				...
				) ICP ON CASE WHEN IP.BRANCHID IS NULL THEN IP.ITEMPRICEID ELSE IP.PARENTITEMPRICEID END = ICP.ITEMPRICEID
...
--Dedutcing quantity from branch quantity      
	...
	FROM 
		...
		INNER JOIN 
		(
			SELECT CASE WHEN IP.BRANCHID IS NULL THEN BRD.ITEMPRICEID ELSE IP.PARENTITEMPRICEID END AS ITEMPRICEID
...
--adding quantity to warehouse      
	...
	FROM 
		...
		INNER JOIN          
			(
				SELECT           
					CASE WHEN IP.BRANCHID IS NULL THEN IP.ITEMPRICEID ELSE IP.PARENTITEMPRICEID END AS ITEMPRICEID
	...
	USING       
		(      
			...
			WHERE      
				...
				AND B.BILLSTATUS = 1 				
			GROUP BY BC.BRANCHID, CASE WHEN IP.BRANCHID IS NULL THEN IP.ITEMPRICEID ELSE IP.PARENTITEMPRICEID END
		) AS Source              
